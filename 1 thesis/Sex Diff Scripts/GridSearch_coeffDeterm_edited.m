function [perform_list, params_list, cv_struc, bestC, bestNu, bestMAE, bestCorr] = GridSearch_coeffDeterm(outcome, features, subjects, C, Nu, k, mode)
...
num_C = length(unique(C)); % Number of unique Cs
num_Nu = length(unique(Nu)); % Number of unique gammas
...
perform_list.MSE = zeros(num_C, num_Nu); 
perform_list.Corr = zeros(num_C, num_Nu);
...
for pc = 1:length(C)
    ...
    for pnu = 1:length(Nu)
        ...
        cmd = ['-s 1 -t 2 -n ', num2str(2^Nu_item), ' -c ', num2str(2^C_item), ' -p 0.1 -q']; % Adjust p for epsilon-value (margin size)
        ...
        if strcmp(mode, 'on') == 1
            fprintf('%g %g %g %g (best C = %g, gamma = %g, MAE = %g, R²-Corr = %g)\n', C_item, Nu_item, MAE, RSquare_Corr, bestC, bestNu, bestMAE, bestCorr);
        end
        perform_list.MAE(pc, pnu) = MAE_cv; % loop1: C1, g1, loop2: C1, g2, ...
        perform_list.Corr(pc, pnu) = Corr_cv;
        params_list(pnu + (pc-1)*num_Nu, 1) = i;
        params_list(pnu + (pc-1)*num_Nu, 2) = C_item;
        params_list(pnu + (pc-1)*num_Nu, 3) = Nu_item;
        params_list(pnu + (pc-1)*num_Nu, 4) = Corr_cv;
        params_list(pnu + (pc-1)*num_Nu, 5) = MSE_cv;
        params_list(pnu + (pc-1)*num_Nu, 6) = MAE_cv;
        cv_struc(i).cv_folds = cv_fold;
        cv_struc(i).C_item = C_item;
        cv_struc(i).G_item = Nu_item;
...
bestNu = 2^params_list(max_idx, 3);
bestCorr = max_Corr;
bestMAE = params_list(max_idx, 6);
fprintf('\n***** Model with best error-correlation-balance selected:');
fprintf('\nC = %g (%g), gamma = %g (%g), MAE = %g (MSE = %g), R²-Corr = %g *****\n', ...
    bestC, params_list(max_idx, 2), bestNu, params_list(max_idx, 3), bestMAE, params_list(max_idx, 5), bestCorr);
end