...
% in stroke patients?
%
% ### MATRIX ADDITION [LESION + WMH MAPS ARE ADDED TO ONE] ###
%
% >> Models:
%       * Model 1 uses Lesion data 
%       * Model 2 uses Lesion + WMH data (added)
%       * Model 3 uses Lesion + permuted WMH data
%       * Model 4 uses Lesion + WMH data (added) + age
%
% >> WMH data formats:
...
elseif (model_version == 2) || (model_version == 3)
    img_dir = fullfile(path, 'Results', ['Model_' num2str(model_version)], 'Addition', 'Images');
end

% Define data type
voxels = 0;
if model_version ~= 1 % Only if WMH data
    fprintf('Data formats: \t*Volumetric data*: 1 - Whole brain, 2 - Left HS, 3 - Right HS\n');
    data_format = input('Select data format. Format # ');
    while (data_format ~= 1) && (data_format ~= 2) && (data_format ~= 3)
       data_format = input('Select data format. Number should be between 1 and 3! Format # ');
    end
...
switch data_format % For structure name later
    case 1
        ...
        elseif model_version == 4
            data_txt = 'Add_VolWB_Age';
        end
    case 2
        if model_version ~= 4
            data_txt = 'Add_VolLHS';
        else
            data_txt = 'Add_VolLHS_Age';
        end
    case 3
        if model_version ~= 4
            data_txt = 'Add_VolRHS';
        else
            data_txt = 'Add_VolRHS_Age';
        end
end
switch voxels 
    case 0
...
elseif model_version == 3 % Permuted WMH data
    n_perm = input('Please enter number of permutation-iterations (default = 5000). n = ...\n');
    if isempty(n_perm) == 1
        n_perm = 5000; % Default
    end
end
...
for p = 1:n_perm
    ...
    if model_version ~= 1
        wmh_s = sum(wmh_all, 1);
        if voxels == 1 % Keep WMH voxels that are damaged in at least 1 patient
            wmh_s_log = (wmh_s > 0);
        elseif voxels == 2 % Keep WMH voxels that are damaged in at least 5 patients
            wmh_s_log = (wmh_s > 4);
        end
    end
    ...
    if model_version ~= 1 % WMH
        maps_s_log = lesions_s_log + wmh_s_log;
        % Add maps together
        lesions_short = lesions_all(:, maps_s_log > 0);
        wmh_short = wmh_all(:, maps_s_log > 0);
        maps_short = lesions_short + wmh_short;
        % Normalize
        maps_max = max(maps_short, [], 'all');
        fprintf('\nMaximum value of added maps: %d\n', maps_max);
        if (maps_max > 1) && ((model_version == 2) || (model_version == 4))
            continue % Skips to next parfor-iteration
        end

        % Control for overlap
        maps_short(maps_short > 1) = 1;
        fprintf('\nMaximum value of corrected added maps: %d\n', max(maps_short, [], 'all'));
    end

    lesions_s = []; lesions_s_log = []; maps_s = []; maps_s_log = [];
    ...
    if pca_log == 1
        ...
        if (model_version ~= 3) && (model_version ~= 4)
            ...
            elseif model_version == 2
                file_pca = [path 'Results\Model_' num2str(model_version) '\Addition\PCA_struc_rs_' num2str(r) '_' data_txt '_' vox_txt '_' t_day '_' t_time '.mat'];
            end
    ...
    % Add age vector to matrix
    if model_version == 4
       maps_short = [maps_short age_all]; 
    end
    ...
    if (model_version == 3) && (p == 1)
        diary([path 'Results\Model_' num2str(model_version) '\Addition\Permutation_output_' num2str(n_perm) '_' data_txt '_' vox_txt '_rs_' num2str(r) '_' t_day '_' t_time '.txt']);
    ...
    if model_version ~= 3 % Only if just one model fitted
        ...
        plot(test_all, y1, 'LineWidth', 2); % Regression line 
        ...
        elseif model_version == 2
            file_nestedCV = [path 'Results\Model_' num2str(model_version) '\Addition\model_struc_nestedCV_' data_txt '_' vox_txt '_rs_' num2str(r) '_avRSquare_' num2str(RSquare_all) '_' t_day '_' t_time '.mat'];
        end
...
if model_version == 3
    ...
    % Save list 
    file_perm = [path 'Results\Model_' num2str(model_version) '\Addition\Permutation_results_' num2str(p) '_' data_txt '_' vox_txt '_Fit_' num2str(perm_selected(1, 3)) '_rs_' num2str(r) '_' t_day '_' t_time '.mat'];
    perm_sort_mat = matfile(file_perm, 'writable', true);
    perm_sort_mat.perm_sort = perm_sort;
    % Save prediction matrix
    file_pred = [path 'Results\Model_' num2str(model_version) '\Addition\Permutation_predictions_' num2str(n_perm) '_' data_txt '_' vox_txt '_Fit_' num2str(perm_selected(1, 3)) '_rs_' num2str(r) '_' t_day '_' t_time '.mat'];
