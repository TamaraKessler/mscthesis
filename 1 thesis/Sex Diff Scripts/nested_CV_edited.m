function [model_struc] = nested_CV(features_rand, outcome_rand, subjects_rand, k, C, Nu, model_version)
...
%                       C - range of values to search for best C
%                       Nu - range of values to search for best gamma
...
for ii = 1:k(1)
    ...
    if model_version == 3 % Silent mode
        [perform_list, params_list, cv_struc, bestC, bestNu, bestMAE, bestCorr] = GridSearch_coeffDeterm_edited(outcome_train, features_train, subjects_train, C, Nu, k(2), 'off'); 
    else
        [perform_list, params_list, cv_struc, bestC, bestNu, bestMAE, bestCorr] = GridSearch_coeffDeterm_edited(outcome_train, features_train, subjects_train, C, Nu, k(2), 'on'); 
    end
        
    % Performance calculation of model selected during inner loop
    options = ['-s 1 -t 2 -p 0.1 -c ' num2str(bestC) '-n ' num2str(bestNu) ' -q']; % Without cv; '-q' to disable screen output    
    ...
    model_struc(ii).bestNu_iL = bestNu;
    ...
    if model_version ~= 3 % Only if not permutation
        ...
        fprintf('\nModel # %d:\ninner loop -> best C = %g, best MAE = %g\nouter loop -> Corr = %f\n', ...
            ii, bestC, bestMAE, accuracy(3));
    end
end
end
